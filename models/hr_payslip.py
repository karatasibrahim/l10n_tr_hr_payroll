    ...
    def onchange_contract(self):
        ...
        if not approved_leaves:
            for default_leave in [
                ...
                worked_days_lines += worked_days_lines.new(default_leave)
            self.worked_days_line_ids = worked_days_lines
        else:
            for default_leave in [
                {
            ...
            normal_mesai_line = self.worked_days_line_ids.filtered(lambda line: line.code == 'NORMAL_MESAI')
            if normal_mesai_line:
                normal_mesai_line.number_of_days = normal_mesai_line.number_of_days - unpaid_leave_days - paid_leave_days
                normal_mesai_line.number_of_hours = normal_mesai_line.number_of_days * 7.5
